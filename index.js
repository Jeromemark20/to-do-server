...
app.post('/add-to-do', (req, res) => {
    ...
    // Validation for 'lists' as a non-empty array
    if (!Array.isArray(lists) || lists.length === 0) {
        return res.status(400).json({ success: false, message: "Lists must be a non-empty array" });
    }

    // Insert the title into the titles table, returning the 'id' column (primary key)
    const titleQuery = "INSERT INTO titles (username, title, date_modified, status) VALUES ($1, $2, $3, $4) RETURNING id"; // 'id' is the primary key in titles
    
    db.query(titleQuery, [username, title, date_modified, status])
      .then(result => {
          const title_id = result.rows[0].id; // Correctly reference 'id' as the primary key in titles

          console.log("Inserted Title ID: ", title_id);

          // Insert tasks into the lists table using the 'title_id'
          ...
          return Promise.all(listQueries);  // Run all insert queries for tasks
      })
      .then(() => {
